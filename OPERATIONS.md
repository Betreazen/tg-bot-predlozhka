...
Clone https://github.com/Betreazen/tg-bot-predlozhka.git and check out a reviewed commit.
...
The integration suite uses `docker-compose.test.yml` with disposable PostgreSQL
and Redis; it must never point at production data. Run `docker compose -f
docker-compose.test.yml up --abort-on-container-exit --exit-code-from tests`.
