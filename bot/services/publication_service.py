...
class PublicationService:
    ...
    async def schedule_publication(
        ...
        scheduled_time = utcnow() + timedelta(seconds=delay_seconds)
    ...
    async def _publish_after_delay(
        ...
        try:
            ...
            # Publish to channel (raises on failure -> handled below).
            ...
            # Route to the retry/notify handler.
            from bot.services.error_handler import get_error_handler
            await get_error_handler().handle_publication_error(submission_id, e, bot)
        finally:
    ...
    async def _publish_to_channel(self, submission, bot: Bot) -> None:
        ...
        Raises:
            Exception: Propagates any send failure so the caller can apply
                retry/notify logic.
        ...
        if not sent_message:
            raise RuntimeError("Channel send returned no message (missing media file_id?)")
